...
*** Send invoice to SdI
Setup a ~\Taocomp\Sdicoop\Client~ object (connect to SdIRiceviFile):
#+BEGIN_SRC 
...
*** Send notice to SdI
Setup a ~\Taocomp\Sdicoop\Client~ object (connect to SdIRiceviNotifica):
#+BEGIN_SRC 
use \Taocomp\Sdicoop\Client;
...
Send notice:
#+BEGIN_SRC 
$fileSdI = new \Taocomp\Sdicoop\FileSdI();
$fileSdI->load($notice);
$response = new \Taocomp\Sdicoop\RispostaSdINotificaEsito($client->NotificaEsito($fileSdI));
#+END_SRC
